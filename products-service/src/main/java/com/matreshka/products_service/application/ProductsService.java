...
import com.matreshka.products_service.internal.infrastructure.exception.ForbiddenException;
...
public class ProductsService implements IProductsService {
    ...
    @Override
    @Transactional
    public AdvertUpdateRequestDTO update(AdvertUpdateRequestDTO advertRequestDTO, String userId) {

        AdvertEntity existing = advertRepo.findById(UUID.fromString(advertRequestDTO.id()))
                .orElseThrow(() -> new NotFoundException("Объявление не найдено"));

        if (!existing.getUserId().equals(userId)) {
            throw new ForbiddenException("У тебя нет прав на данное действие");
        }

        advertEntityMapper.toEntity(advertRequestDTO, existing);

        if (existing.getPictures() != null) {
            existing.getPictures().forEach(picture -> picture.setAdvert(existing));
        }
        if (existing.getServices() != null) {
            existing.getServices().forEach(service -> service.setAdvert(existing));
        }
        if (existing.getWorkSchedule() != null) {
            existing.getWorkSchedule().forEach(schedule -> schedule.setAdvert(existing));
        }

        AdvertDocument advertDocument = advertEntityMapper.toDocument(existing);
        advertRepo.save(existing);
        advertSearchRepo.save(advertDocument);

        return advertEntityMapper.toUpdateRequestDTO(existing);
    }
    ...
    @Override
    @Transactional
    public void updateVideoId(UpdateVideoInAdvertRequestDTO updateVideoInAdvertRequestDTO, String userId) {
        try {
            AdvertEntity advertEntity = advertRepo.findById(updateVideoInAdvertRequestDTO.id())
                    .orElseThrow(() -> new NotFoundException("Товар не найден"));

            if(!advertEntity.getUserId().equals(userId)){
                throw new ForbiddenException("Нет доступа к редактированию объявления");
            }
            ...
            if(!mediaEntity.getUserId().equals(userId)){
                throw new ForbiddenException("Нет доступа к редактированию видео");
            }
